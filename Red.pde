...
// Música
Minim minim;
AudioPlayer musicaBatalla;
AudioPlayer musicaMenu;
AudioPlayer GameOver;
AudioPlayer victory;
AudioPlayer Sonidoboton;
AudioPlayer golpe;
...
void setup() {
  ...
  musicaMenu = minim.loadFile("musica/MenuTheme.mp3");
  GameOver  = minim.loadFile("musica/GameOver.mp3");
  victory = minim.loadFile("musica/victory.mp3");
  Sonidoboton = minim.loadFile("musica/boton.mp3");
  golpe = minim.loadFile("musica/golpe.mp3");
  musicaMenu.loop();
...
void draw() {
  if (menu == true) {
    /*
     ...
     Con esta función también se dibuja el fondo por algunos cambios
     */
    lluvia();
